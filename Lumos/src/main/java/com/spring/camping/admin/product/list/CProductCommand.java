...
public class CProductCommand implements ICommand {
	...
	public void execute(SqlSession sqlSession, HttpServletRequest request, Model model, HttpSession session)
		...
		System.out.println("CProductCommand()");
		...
		int startNum=((currentPage-1)*pageSize);//전체 페이지 개수	
		
		
		AdminProductList_IDao dao = sqlSession.getMapper(AdminProductList_IDao.class);
		model.addAttribute("productlist", dao.productlistDao(startNum));
